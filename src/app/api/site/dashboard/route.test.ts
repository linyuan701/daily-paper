...
vi.mock("../../../../lib/http/cloudflare-access", () => ({ verifyCloudflareAccess: mocks.access }));
...
describe("GET /api/site/dashboard read-only route", () => {
  it("rejects query parameters without reading the database", async () => {
    const response = await GET(new Request("https://daily.example/api/site/dashboard?runId=arbitrary"));
  ...
  it.each(["https://daily.example", "http://localhost", "http://127.0.0.1"])("rejects unauthenticated %s before creating a database client", async (origin) => {
