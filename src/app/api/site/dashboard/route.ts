...
export async function GET(request: Request) {
  return handleSiteDashboard(request, { verifyAccess: verifyCloudflareAccess, readDashboard: getSiteDashboard });
}

export async function handleSiteDashboard(request: Request, dependencies: Dependencies) {
  ...
  if (new URL(request.url).search) {
    return NextResponse.json({ status: "error", code: "INVALID_QUERY" }, { status: 400, headers });
  }
  try {
...
const rejectMethod = (request: Request) => handleSiteDashboard(request, {
  verifyAccess: verifyCloudflareAccess, readDashboard: getSiteDashboard
});
